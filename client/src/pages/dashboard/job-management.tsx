...
import { getApplyByStudent } from '../../../services/job'
...
function JobManagement({ user }: InferGetServerSidePropsType<GetServerSideProps>) {
  const [applys, setApplys] = useState<any[]>()
  const [isLoading, setIsLoading] = useState(false)

  useEffect(() => {
    setIsLoading(true)
    getApplyByStudent().then(res => setApplys(res)).catch(err => console.log(err)).finally(() => setIsLoading(false))
  }, [])
      ...
      {isLoading ? <Loader className='mx-auto' /> : (
        <div>
          <div className='flex px-4 py-4 gap-2 items-center'>
            <IconCheck size={30} className='text-[#005247]' />
            <h1 className="flex text-[#005247] text-xl font-bold items-center gap-2 justify-between">Việc làm đã ứng tuyển</h1>
          </div>
          <Table striped highlightOnHover withBorder withColumnBorders>
            ...
            <tbody className='text-center'>
              {
                applys?.map((element: any, index: number) => (
                  <tr key={element.job.id} >
                    ...
                    {/* <td className='font-semibold text-gray-700'>{element.status}</td> */}
                  </tr>
                ))
              }
            </tbody>
