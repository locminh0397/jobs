...
import { uploadImage } from '@/ultil';
import { saveAvatar } from '../../../services/student';
...
export function UploadAvatar({ user }: Props) {
  ...
  return (
    <div className=' bg-white profile mt-4 shadow-md py-2 mb-4'>
      <div className='flex px-4 gap-2 items-center'>
        <IconPhoto size={30} className='text-[#005247]' />
      ...
      <div className="p-4 flex gap-4">
        <div className='w-[300px] h-[300px] rounded-full border border-dotted flex justify-center items-center bg-gray-200'>
          {files.length > 0 ? previews : user.avatar_url ? <Image src={user.avatar_url} /> : <p className='text-2xl'>Ảnh đại diện</p>}
        </div>
        ...
        <div className={`${classes.wrapper} flex-1`}>
          ...
          >
            <div style={{ pointerEvents: 'none' }}>
              ...
              <Text ta="center" fw={700} fz="lg" mt="xl">
                ...
                <Dropzone.Idle>Chọn ảnh đại diện</Dropzone.Idle>
              </Text>
          ...
          <Button disabled={isLoading} onClick={handleUploadImage} variant='outline' className='mt-4 ml-auto block' size="md" radius="xl" type='button'>
            {isLoading ? "Đang tải lên..." : " Cập nhật ảnh đại diện"}
          </Button>
        </div>
